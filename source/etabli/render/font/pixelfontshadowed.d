 ...
 * Auteur: Enalye
 */
module etabli.render.font.pixelfontshadowed;
...
final class PixelFontShadowed : PixelFont {
    ...
    @property {
        /// Taille de la police
        int size() const {
            return (_ascent - _descent) * _weight + 1;
        }
    ...
    this(int ascent_, int descent_, int lineSkip_, int weight = 1, int spacing_ = 0) {
        ...
        _unknownGlyph = new PixelGlyphShadowed();
    ...
    override void addCharacter(dchar ch, int[] glyphData, int width, int height, int descent) {
        ...
        if (_posX + width * _weight * 2 + 1 >= _surfaceW) {
            _posX = 0;
            _posY += (_ascent - _descent) * _weight * 2 + 1;

            if (_posY + (_ascent - _descent) * _weight * 2 + 1 > _surfaceH) {
                _posY = 0;
                _texture = new WritableTexture(_surfaceW, _surfaceH);
            }
        }
                    ...
                    if (data.glyph[iy] & mask) {
                        for (int ry; ry < data.weight; ++ry) {
                            for (int rx; rx < data.weight; ++rx) {
                                ...
                                dest[indexBorder + texWidth] = 0xFFFFFFFF;
                                dest[indexBorder + 1] = 0xFFFFFFFF;
                            }
                        }
                    }
                    mask >>= 1;
...
private final class PixelGlyphShadowed : Glyph {
    @property {
    ...
    void draw(Vec2f position, float scale, Color color, float alpha) {
        ...
        _imageData.draw(position, Vec2f((_width + 1) * scale,
                (_height + 1) * scale), Vec4u(_x + _width, _y, _width + 1, _height + 1), 0f);
