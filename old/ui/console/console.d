...
module etabli.ui.console.console;

import etabli.common, etabli.core, etabli.render;
import etabli.ui.list, etabli.ui.gui_element, etabli.ui.layout, etabli.ui.inputfield;
import etabli.ui.gui_overlay, etabli.ui.text, etabli.ui.gui_manager;
/*
private {
	Console _console;
...
void addConsoleCmd(string cmd, ElementCallback callback) {
	if(!_console)
		return;
...
private class ConsoleHandler: UIElement {
	private {
...
private class Console: AnchoredLayout {
	private {
		...
		ElementCallback[string] _elementCallbacks;
	...
	this() {
		...
		addNodeGui(_log, Vec2f(.5f, .5f - inputFieldRatio / 2f), Vec2f(1f, 1f - inputFieldRatio));
		addNodeGui(_inputField, Vec2f(.5f, 1f - inputFieldRatio / 2f), Vec2f(1f, inputFieldRatio));
	}
	...
	void addCmd(string cmd, ElementCallback callback) {
		_elementCallbacks[cmd] = callback;
	}
	...
	protected void parse(string text) {
		...
		if(!parameters.length)
			return;
		auto elementCallback = parameters[0] in _elementCallbacks;
		if(elementCallback) {
			Event event;
			event.type = EventType.callback;
			event.id = (*elementCallback).id;
			event.sarray = parameters[1..$];
			(*elementCallback).element.onEvent(event);
			return;
		}
